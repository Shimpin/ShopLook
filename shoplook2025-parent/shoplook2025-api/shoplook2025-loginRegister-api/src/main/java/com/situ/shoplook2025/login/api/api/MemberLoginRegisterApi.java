package com.situ.shoplook2025.login.api.api;
...
import com.situ.shoplook2025.login.api.dao.SmsSender;
...
//        //req.getSession().setAttribute("captcha", captcha.text().toLowerCase());
    ...
    //手机短信验证码
    @GetMapping("/code")
    public ResponseEntity<JsonResult<?>> getCode(String phone) {
        try {
            // 生成6位随机数字验证码
            ...
            // 发送短信验证码
            SmsSender.sendSms(phone, code, "CST_ptdie100");

            return ResponseEntity.ok(JsonResult.success("验证码发送成功"));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.ok(JsonResult.fail("验证码发送失败"));
        }
    }
        ...
        String jwt = JwtUtils.createJwt(
                ...
                user.getPhone() != null ? user.getPhone() :
                        user.getEmail() != null ? user.getEmail() :
                                user.getAccount(),
                Map.of(
                        "username", user.getEmail() != null ? user.getEmail() : user.getAccount(),
                        "userId", user.getId()
                ),
    ...
    public ResponseEntity<JsonResult<?>> phoneLogin(String phone, String code, HttpServletRequest request) {
        try {
            // 从Redis获取存储的验证码
            String correctCode = (String) redisTemplate.opsForValue().get("sms-code-" + phone);

            // 验证验证码是否正确
            if (correctCode == null || !correctCode.equals(code)) {
                return ResponseEntity.ok(JsonResult.fail("验证码错误或已过期"));
            }

            // 查找用户
            Member user = memberFeignService.findByPhone(phone);
            ...
            String jwt = JwtUtils.createJwt(
                    ...
                    user.getPhone(),
                    Map.of(
                            "username", user.getPhone() != null ? user.getPhone() : user.getAccount(),
                            "userId", user.getId()
                    ),
                    LocalDateTime.now().plusMinutes(30),
                    jwtSecret
            ...
            e.printStackTrace();
            return ResponseEntity.ok(JsonResult.fail("登录失败"));
        }
    }

    @PostMapping("/register")
    public ResponseEntity<JsonResult<?>> save(@RequestBody @Validated Member member,String code,HttpServletRequest request) {
        ...
        if (!codeService.validateCode(member.getEmail(), code)) {
    ...
    public ResponseEntity<JsonResult<?>> savePhone(@RequestBody @Validated Member member,String code,HttpServletRequest request) {
        String correctCode = (String) redisTemplate.opsForValue().get("sms-code-" + member.getPhone());
